...
// WithInstanceID sets an instance identifier that is prepended to the random
// worker ID generated by each Acquire call. This makes worker IDs correlatable
// to a specific process or pod (e.g., "compass-7d8f9b6c4-x2k9p:a1b2c3d4e5f6").
//
// When not set, worker IDs are random hex strings only (backward compatible).
//
// Typical usage reads the Kubernetes POD_NAME environment variable:
//
//	distributed.WithInstanceID(os.Getenv("POD_NAME"))
